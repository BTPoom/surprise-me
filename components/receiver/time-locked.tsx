...
import { Lock, Unlock, Calendar, Sparkles, AlertTriangle } from "lucide-react";
...
export function TimeLocked({
  ...
  const target = useMemo(() => new Date(unlockAt), [unlockAt]);
  const isValidDate = !isNaN(target.getTime());

  const diff = now && isValidDate ? target.getTime() - now.getTime() : Infinity;
  const isUnlocked = isValidDate && diff <= 0;
  ...
  // หยุดนับถอยหลังทันทีที่ปลดล็อกแล้ว ไม่ต้อง tick ต่อไปเรื่อย ๆ
  useEffect(() => {
    if (!isClient || !isValidDate || isUnlocked) return;
    const interval = setInterval(() => setNow(new Date()), 1000);
    return () => clearInterval(interval);
  }, [isClient, isValidDate, isUnlocked]);

  const formatCountdown = useCallback(() => {
    if (diff <= 0 || diff === Infinity) return { d: 0, h: 0, m: 0, s: 0 };
  ...
  // unlockAt parse ไม่ได้ (prop ผิด/ไม่ครบ) — โชว์ error แทนการพังเงียบ ๆ
  if (!isValidDate) {
    return (
      <div className={`w-full max-w-md mx-auto rounded-3xl border border-red-200 bg-red-50 p-6 text-center ${className || ""}`}>
        <AlertTriangle className="w-8 h-8 text-red-400 mx-auto mb-2" />
        <p className="text-sm text-red-500">ตั้งเวลาปลดล็อกไม่ถูกต้อง</p>
      </div>
    );
  }
