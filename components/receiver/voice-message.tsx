...
import { Play, Pause, Volume2, VolumeX, Phone, Mic, MessageCircle, Loader2 } from "lucide-react";
...
export function VoiceMessage({
  ...
  // ผูก volume state กับ audio element จริง (ก่อนหน้านี้เป็น dead state)
  useEffect(() => {
    const audio = audioRef.current;
    if (!audio) return;
    audio.volume = volume;
  ...
  // Visualizer — แก้ให้คมชัดบนจอ retina ด้วย devicePixelRatio
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bars; i++) {
        const h = Math.random() * rect.height * 0.8 + rect.height * 0.1;
        ...
        <div className="bg-gray-800 rounded-3xl p-5 shadow-xl border border-gray-700">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5">
              <button onClick={toggleMute} aria-label={isMuted ? "เปิดเสียง" : "ปิดเสียง"} className="text-gray-400 hover:text-white transition-colors">
              ...
              <input
                ...
                className="w-14 h-1 bg-gray-600 rounded-full appearance-none cursor-pointer accent-amber-500"
              />
            </div>
          </div>
        </div>
    ...
    <div className={`w-full max-w-sm mx-auto ${className || ""}`}>
      <div className={`rounded-3xl p-5 shadow-lg border ${t.split(" ")[2]} bg-white`}>
        ...
        <div className="flex justify-between mt-2">
          <span className="text-[10px] text-gray-400">{formatTime(currentTime)}</span>
          <button onClick={toggleMute} aria-label={isMuted ? "เปิดเสียง" : "ปิดเสียง"} className="text-gray-300 hover:text-gray-500 transition-colors">
