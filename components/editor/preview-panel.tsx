...
import { X, Smartphone, Tablet, Monitor, Mail, MailOpen, Image as ImageIcon, Music, MessageCircle, Film, Mic, Lock } from "lucide-react";
...
export function PreviewPanel({ data, onClose }: { data: EditorData; onClose: () => void }) {
  ...
  const mockPage = {
    ...
    videoStyle: data.videoStyle || "film",
    voiceUrl: data.voiceUrl || null,
    voiceStyle: data.voiceStyle || "cassette",
    secretUnlockAt: data.secretUnlockAt || null,
    secretMessage: data.secretMessage || null,
  ...
  const hasVideo = Boolean(mockPage.videoUrl);
  const hasVoice = Boolean(mockPage.voiceUrl);
  const hasSecret = Boolean(mockPage.secretUnlockAt && mockPage.secretMessage);
  ...
  const jumpSteps: { key: string; label: string; icon: any; go: () => void }[] = [
    ...
    ...(hasVideo ? [{ key: "video", label: "วิดีโอ", icon: Film, go: () => { setPhase("content"); setScrollTarget("video"); } }] : []),
    ...(hasVoice ? [{ key: "voice", label: "เสียง", icon: Mic, go: () => { setPhase("content"); setScrollTarget("voice"); } }] : []),
    ...(hasSecret ? [{ key: "secret", label: "ข้อความลับ", icon: Lock, go: () => { setPhase("content"); setScrollTarget("secret"); } }] : []),
